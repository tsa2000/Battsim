...
class EKF:
    ...
    def __init__(
        ...
        # Plett 2004 Table I: P_SOC = 0.01 → ±10% initial 1σ uncertainty
        # P_RC: small (RC voltages start near 0 with high confidence)
        self.P = np.diag([
            0.01,    # SOC variance [dimensionless²] — Plett 2004, Table I
            1e-6,    # V_RC1 variance [V²]
            1e-6,    # V_RC2 variance [V²]
        ])
        ...
        # ── Measurement noise R ───────────────────────────────────────────────
        # Initialised from sensor noise_var; adapted online (Sage-Husa 1969)
        self._R_base  = r_scale * noise_var
        self._R_adapt = self._R_base
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Adaptive R — Sage & Husa (1969), corrected form:
    #   R_{k+1} = β·R_k + (1−β)·(νₖ² − H·P⁻·Hᵀ)
    # Subtracting H·P⁻·Hᵀ removes the predicted covariance contribution
    # from the R estimate, giving an unbiased innovation-based update.
    # Reference: Sage & Husa 1969 · Yue et al. 2026 (eIAEKF)
    # ─────────────────────────────────────────────────────────────────────────
    def _adapt_R(self, nu: float, Hk: np.ndarray, Pp: np.ndarray) -> None:
        S_k = float((Hk @ Pp @ Hk.T)[0, 0])
        self._R_adapt = (
            self._beta * self._R_adapt
            + (1.0 - self._beta) * (nu ** 2 - S_k)
        )
        self._R_adapt = float(np.clip(self._R_adapt, 1e-8, 1e-1))
        self.R = np.array([[self._R_adapt]])
    ...
    def step(
        ...
        # No clipping here — clipping in predict corrupts the Jacobian
        # and biases the P matrix. (Plett 2004, Section 4.3)
        ...
        # Adaptive R — Sage-Husa corrected form (unbiased)
        self._adapt_R(nu, Hk, P_p)
